...
const getGreeting = (language: string) => {
  switch (language) {
    case 'te': return 'నమస్కారం, రైతు గారు!';
    case 'hi': return 'नमस्कार, किसान जी!';
    default: return 'Welcome, Farmer!';
  }
};

const getSubtitle = (language: string) => {
  switch (language) {
    case 'te': return 'మీ సేంద్రీయ వ్యవసాయ సహాయకుడు';
    case 'hi': return 'आपका जैविक खेती सहायक';
    default: return 'Your Organic Farming Assistant';
  }
};

export default function Dashboard({ onFeatureSelect, selectedLanguage, isActive }: DashboardProps) {
  const { t, language } = useI18n();
  const features = [
    { id: 'chatbot', icon: MessageCircle, title: t('aiChatBotTitle'), subtitle: t('aiChatBotDesc'), description: t('aiChatBotDesc'), color: 'bg-gradient-to-br from-pink-500 to-pink-600' },
    { id: 'guides', icon: BookOpen, title: t('solutionsTitle'), subtitle: t('solutionsDesc'), description: t('solutionsDesc'), color: 'bg-gradient-to-br from-blue-500 to-blue-600' },
    { id: 'calendar', icon: Calendar, title: t('advisoryCalendar'), subtitle: t('planAdvisorySub'), description: t('planAdvisorySub'), color: 'bg-gradient-to-br from-orange-500 to-orange-600' },
    { id: 'tracking', icon: ClipboardList, title: t('mySuccessLogs'), subtitle: t('trackBest'), description: t('trackBest'), color: 'bg-gradient-to-br from-indigo-500 to-indigo-600' },
    { id: 'community', icon: Users, title: t('communityHeader'), subtitle: t('communitySub'), description: t('communitySub'), color: 'bg-gradient-to-br from-purple-500 to-purple-600' },
    { id: 'suppliers', icon: MapPin, title: t('localSuppliers'), subtitle: t('findSuppliersSub'), description: t('certifiedSuppliers'), color: 'bg-gradient-to-br from-teal-500 to-teal-600' },
  ];

  const [weather, setWeather] = useState<{ temp: number; city: string; country: string; area?: string } | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [timeInfo, setTimeInfo] = useState<{ time: string; day: string; date: string }>({
    time: '',
    day: '',
    date: '',
  });
        ...
        async (position) => {
          try {
            ...
            setWeather({
              ...
              city: geoData.address.city || geoData.address.town || geoData.address.village || weatherData.name,
    ...
    <div ref={containerRef} className="min-h-screen bg-primary-50">
      ...
      <div className="bg-gradient-to-br from-primary-500 to-primary-700 text-white px-4 pt-12 pb-6">
        <div className="max-w-2xl mx-auto">
          <h1 className="text-2xl sm:text-3xl font-bold mb-2">{getGreeting(selectedLanguage)}</h1>
          <p className="text-primary-100 text-sm sm:text-base mb-4">{getSubtitle(selectedLanguage)}</p>
            ...
            <div className="bg-white bg-opacity-20 rounded-lg p-4 mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between shadow-md">
              <div className="mb-3 sm:mb-0">
                <p className="text-lg font-semibold">
                  📍 {weather.area ? `${weather.area}, ` : ''}{weather.city}, {weather.country}
                </p>
                <p className="text-sm text-primary-100">{t('currentLocation')}</p>
              </div>
              <div className="flex flex-col sm:items-end text-primary-50">
                ...
                <p className="text-sm text-primary-100">{timeInfo.day}, {timeInfo.date}</p>
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-white bg-opacity-20 rounded-lg p-3 text-center">
              {weather ? (
                <>
                  ...
                  <div className="text-xs text-primary-100 mt-1">{t('cloudy')}</div>
                </>
              ) : error ? (
            ...
            <div className="bg-white bg-opacity-20 rounded-lg p-3 text-center">
              ...
              <div className="text-xs text-primary-100">{t('activeAlertsStat')}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto p-4">
        ...
        <div className="mb-6">
          ...
          >
            <div className="bg-gradient-to-br from-green-500 to-green-600 p-6 text-white">
              ...
              <h3 className="text-2xl font-bold mb-2">{t('aiPhotoAnalysis')}</h3>
        ...
        {/* All Features */}
        <div>
          <h2 className="text-lg font-bold text-primary-500 mb-4">{t('allFeatures')}</h2>
            ...
            {features.map((feature) => {
              ...
              return (
                ...
                >
                  ...
                  <h3 className="font-semibold text-primary-600 text-sm mb-1">{feature.title}</h3>
